...
def run_clang_tidy(clang_tidy, path, build_path, apply_tidy, verbose):
    if apply_tidy:
        cmd = clang_tidy + ' -p=' + build_path + ' -fix ' + path
    else:
        cmd = clang_tidy + ' -p=' + build_path + ' ' + path #+ ' -dump-config'
...
def run(clang_tidy_path, file_paths, build_path, apply_tidy, verbose):
    result = True
    for p in file_paths:
        _, ext = os.path.splitext(p)
        if ext in file_extensions:
            r = run_clang_tidy(clang_tidy_path, p, build_path, apply_tidy, verbose)
            if not r:
                result = False
    return result
...
def main():
    ...
    result = run(clang_tidy_path, file_paths, project_build_path, apply_tidy, verbose)
