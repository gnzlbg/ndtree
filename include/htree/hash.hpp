#pragma once
/// \file hash.hpp Spatial hash for each position within the tree
...
template <int nd, int max_no_levels = 8> struct node_hash {
  static_assert(max_no_levels < 256, "");
  unsigned char level;
  unsigned bits : 3;
};
static_assert(sizeof(node_hash<1>) == 4, "");

HTREE_ASSERT_STANDARD_LAYOUT(node_hash<1>);
...
//   }
//   static_assert(nd > 0 and nd <= 3, "unsupported dimension for geohash
//   piece");
