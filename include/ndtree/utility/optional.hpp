...
template <class T, class U> struct is_assignable {
  template <class X, class Y> static constexpr bool has_assign(...) {
    return false;
  }
...
template <class T> struct is_nothrow_move_assignable {
  template <class X, bool has_any_move_massign> struct has_nothrow_move_assign {
...
template <typename T> struct has_overloaded_addressof {
  template <class X> static constexpr bool has_overload(...) { return false; }
...
template <class U> struct is_not_optional {
  constexpr static bool value = true;
};

template <class T> struct is_not_optional<optional<T>> {
  constexpr static bool value = false;
};
...
template <class T> struct optional_base {
  ...
  template <class U, class... Args,
            TR2_OPTIONAL_REQUIRES(
             is_constructible<T, std::initializer_list<U>>)>
...
template <class T> class optional<T&> {
 ...
 public:
  ...
  void emplace(T& v) noexcept { ref = static_addressof(v); }
...
namespace std {
template <typename T> struct hash<std::experimental::optional<T>> {
